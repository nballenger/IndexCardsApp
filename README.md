...
6. **It doesn't have AI, but feel free to bring your own.** - I can't imagine a good reason for the application itself to have AI built directly into it. That said, if you want to have an AI tool drive it, be my guest. I've tried my best to make the documents their own self-contained units of meaning, so you _should_ be able to give one to an AI agent and have it reason well about the content. Additionally the app has a few command-line flags for an external agent, none of which open the GUI: `indexcards --format-guide` prints the file format guide, `indexcards --schema` prints its JSON Schema, and `indexcards --validate <file>` checks a file it wrote. If you use Claude Code, `skills/indexcards/` is a small skill that wires those together; copy that folder into `~/.claude/skills/` and Claude will know how to build and check boards. 
